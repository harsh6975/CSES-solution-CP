...
void EulerPath(vvl &graph, ll sc, vl &ans)
{
    while (graph[sc].size() > 0)
    {
        ll x = graph[sc].back();
        graph[sc].pop_back();
        EulerPath(graph, x, ans);
    }
    ans.push_back(sc + 1);
}

void dfs(vvl &graph, vl &vis, ll sc)
{
    vis[sc] = 1;
    for (auto x : graph[sc])
    {
...
int main()
{
    ...
    if ((indeg[0] - outdeg[0] == 1 && indeg[n - 1] - outdeg[n - 1] == -1) || (indeg[0] - outdeg[0] == -1 && indeg[n - 1] - outdeg[n - 1] == 1))
    {
    }
    else
    {
