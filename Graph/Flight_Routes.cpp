...
void dijkstra(vvpl &graph, vvl &dis, int sc)
{
    int n = graph.size();
    int k = dis[0].size();
    vl vis(n);
    ...
    while (!q.empty())
    {
        auto curr = q.top();
        q.pop();
        if(dis[curr.second][k-1]<curr.first)continue;
        for (auto x : graph[curr.second])
        {
            if (dis[x.first][k - 1] > curr.first + x.second)
            {
                dis[x.first][k - 1] = curr.first + x.second;
                q.push({dis[x.first][k - 1], x.first});
                sort(all(dis[x.first]));
            }
        }
    }
}
