...
void cal_indp(vvl &graph, vl &inDp, vl &szi, int sc, int par)
{
    szi[sc] = 1;
    inDp[sc] = 0;
    for (auto x : graph[sc])
    {
        if (x == par)
            continue;
        cal_indp(graph, inDp, szi, x, sc);
        szi[sc] += szi[x];
        inDp[sc] += inDp[x] + szi[x];
    }
}

void cal_outdp(vvl &graph, vl &inDp, vl &outDp, vl &szi, int sc, int par)
{
    int n = sz(graph);
    if (par != -1)
    {
        outDp[sc] = (outDp[par] + n - szi[par]) + (inDp[par] - inDp[sc] - szi[sc]) + szi[par] - szi[sc];
    }
    ...
    for (auto x : graph[sc])
    {
        if (x == par)
            continue;
        ;
        cal_outdp(graph, inDp, outDp, szi, x, sc);
    }
}
...
int main()
{
    ...
    vector<ll> inDp(n), outDp(n), szi(n);
    cal_indp(graph, inDp, szi, 0, -1);
    cal_outdp(graph, inDp, outDp, szi, 0, -1);
    FOR0(i, n)
    {
        cout << inDp[i] + outDp[i] << " ";
    }
