...
ll startIndex[MAX], endIndex[MAX], euler[MAX], segTree[4 * MAX];
vl value;
...
int main()
{
    ...
    while (q--)
    {
        ...
        {
            cin >> b;
            update(root, 0, 2 * n - 1, startIndex[a], b);
            update(root, 0, 2 * n - 1, endIndex[a], b);
        }
        else
        {
            cout << find(root, 0, 2 * n - 1, startIndex[a], endIndex[a]) / 2 << '\n';
        }
    }
